...
describe("local agent PAPERCLIP_API_KEY injection", () => {
  ...
  afterAll(async () => {
    ...
    if (db) {
      try {
        ...
        // Pool may already be disconnected if beforeAll failed mid-setup.
      }
    }
    if (embeddedPostgres) {
  ...
  async function runWakeCase(input: {
    ...
    requireEligibleTimerIssue?: boolean;
    projectWorkspace?: {
      cwd: string;
      repoRef?: string | null;
    };
    ...
    if (input.wake.source !== "timer" || input.requireEligibleTimerIssue) {
      issueId = await seedIssue(agentId, {
        projectId,
        projectWorkspaceId,
      });
    }
    if (input.wake.reason === "issue_comment_mentioned" && issueId) {
      wakeCommentId = await db
        .insert(issueComments)
        .values({
          companyId,
          issueId,
          authorUserId: "user-1",
          body: `@${agentName} please inspect this wake.`,
        })
        .returning()
  ...
  for (const adapterType of ["codex_local", "claude_local"] as const) {
    describe(adapterType, () => {
      it("injects PAPERCLIP_API_KEY on eligible timer wakes", async () => {
        const { capture, run } = await runWakeCase({
          adapterType,
          wake: { source: "timer", triggerDetail: "system" },
          requireEligibleTimerIssue: true,
        });
      ...
      it("injects shared project workspace context for project-backed wakes", async () => {
        ...
        try {
          ...
          expect(capture.env.PAPERCLIP_WORKSPACE_OBSERVED_BRANCH).toBeUndefined();
          expect(capture.env.PAPERCLIP_WORKSPACE_OBSERVED_HEAD).toBeUndefined();
