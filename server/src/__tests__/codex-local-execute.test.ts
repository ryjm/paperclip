...
describe("codex execute", () => {
  ...
  it("starts a fresh session when managed skill sources change between wakes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-codex-execute-resume-changed-skill-source-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const instructionsPath = path.join(root, "AGENTS.md");
    const managedSkillSourceA = await writeManagedSkillSource(root, "managed-skill-a");
    const managedSkillSourceB = await writeManagedSkillSource(root, "managed-skill-b");
    await fs.mkdir(workspace, { recursive: true });
    await fs.writeFile(instructionsPath, "You are managed instructions.\n", "utf8");
    await writeFakeCodexCommand(commandPath);
    ...
    const wakeContext = {
      ...
      wakeCommentId: "comment-2",
      paperclipWake: {
        reason: "issue_commented" as const,
        issue: {
          id: "issue-1",
          identifier: "PAP-874",
          title: "chat-speed issues",
    ...
    const runWithSkillSource = async (input: {
      capturePath: string;
      skillSource: string;
      ...
      let promptMetrics: Record<string, number> = {};
      const result = await execute({
        runId: `run-${path.basename(input.capturePath, ".json")}`,
        agent: {
          id: "agent-1",
          companyId: "company-1",
          name: "Codex Coder",
        ...
        runtime: {
          sessionId: null,
          sessionParams: input.runtimeSessionParams,
          sessionDisplayId: null,
          taskKey: null,
        },
        config: {
          ...
          paperclipSkillSync: {
            desiredSkills: ["managed-skill"],
          },
          promptTemplate: "Follow the paperclip heartbeat.",
        },
        context: input.runtimeSessionParams ? wakeContext : {},
        authToken: "run-jwt-token",
        onLog: async (stream, chunk) => {
          logs.push({ stream, chunk });
        },
      ...
      const capture = JSON.parse(await fs.readFile(input.capturePath, "utf8")) as CapturePayload;
      return {
        capture,
        invocationNotes,
        logs,
    ...
    try {
      const firstRun = await runWithSkillSource({
        capturePath: path.join(root, "capture-first.json"),
        skillSource: managedSkillSourceA,
        runtimeSessionParams: null,
      });
      expect(firstRun.capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      const initialSessionParams = firstRun.result.sessionParams as Record<string, unknown>;
      expect(initialSessionParams.paperclipManagedSkillFingerprint).toEqual(expect.any(String));

      const unchangedRun = await runWithSkillSource({
        capturePath: path.join(root, "capture-unchanged.json"),
        skillSource: managedSkillSourceA,
        runtimeSessionParams: initialSessionParams,
      });
      expect(unchangedRun.capture.argv).toEqual(expect.arrayContaining(["resume", "codex-session-1", "-"]));
      expect(unchangedRun.capture.prompt).toContain("## Paperclip Resume Delta");
      expect(unchangedRun.invocationNotes).toContain(
        "Skipped stdin instruction reinjection because an existing Codex session is being resumed with a wake delta.",
      );

      const changedRun = await runWithSkillSource({
        capturePath: path.join(root, "capture-changed.json"),
        skillSource: managedSkillSourceB,
        runtimeSessionParams: initialSessionParams,
      });
      expect(changedRun.capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      expect(changedRun.capture.argv).not.toEqual(expect.arrayContaining(["resume", "codex-session-1", "-"]));
      expect(changedRun.capture.prompt).toContain("Follow the paperclip heartbeat.");
      expect(changedRun.capture.prompt).toContain("You are managed instructions.");
      expect(changedRun.capture.prompt).not.toContain("## Paperclip Resume Delta");
      expect(changedRun.invocationNotes).not.toContain(
        "Skipped stdin instruction reinjection because an existing Codex session is being resumed with a wake delta.",
      );
      expect(changedRun.promptMetrics.instructionsChars).toBeGreaterThan(0);
      expect(changedRun.promptMetrics.heartbeatPromptChars).toBeGreaterThan(0);
      expect(changedRun.logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("was saved with different managed skills and will not be resumed"),
        }),
      );
      expect(
        (changedRun.result.sessionParams as Record<string, unknown>).paperclipManagedSkillFingerprint,
      ).not.toEqual(initialSessionParams.paperclipManagedSkillFingerprint);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      await fs.rm(root, { recursive: true, force: true });
