...
GitHub evidence is required before an issue can move to `done` when the issue has the `code` label.
...
- Coordination-only tasks without the `code` label do not need GitHub evidence.
