...
When a project workspace has a local `cwd`, Paperclip materializes a task-scoped workspace under that same agent home instead of pointing multiple runs at the shared source checkout. Git-backed project folders use `git worktree` when available; non-git folders fall back to an isolated directory copy.
