...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Must be done before prepareOpenCodeRuntimeConfig so the derived value is included in the
  // prepared runtime env.
  const instructionsFilePathForAgentHome = asString(config.instructionsFilePath, "").trim();
  if (!env.AGENT_HOME && instructionsFilePathForAgentHome) {
    env.AGENT_HOME = path.dirname(path.resolve(cwd, instructionsFilePathForAgentHome));
  }
